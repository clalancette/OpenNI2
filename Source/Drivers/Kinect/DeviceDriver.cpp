...
{
class Device final : public oni::driver::DeviceBase, public Freenect::FreenectDevice
{
public:
	...
	bool isImageRegistrationModeSupported(OniImageRegistrationMode mode) override { return depth->isImageRegistrationModeSupported(mode); }
	...
	OniStatus getProperty(int propertyId, void* data, int* pDataSize) override
	{
		switch (propertyId)
		{
			...
			case ONI_DEVICE_PROPERTY_PLAYBACK_REPEAT_ENABLED: // bool
			// xn
...
class Driver final : public oni::driver::DriverBase, private Freenect::Freenect
{
public:
	...
	OniStatus initialize(oni::driver::DeviceConnectedCallback connectedCallback, oni::driver::DeviceDisconnectedCallback disconnectedCallback, oni::driver::DeviceStateChangedCallback deviceStateChangedCallback, void* pCookie) override
	{
		...
		for (int i = 0; i < Freenect::deviceCount(); i++)
		{
			...
			OniDeviceInfo info;
			strncpy(info.uri, uri.c_str(), ONI_MAX_STR - 1);
			info.uri[ONI_MAX_STR - 1] = '\0';
	...
	oni::driver::DeviceBase* deviceOpen(const char* uri, const char* mode = NULL) override
	{
		(void)mode;
